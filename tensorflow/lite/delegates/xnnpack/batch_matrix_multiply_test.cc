...
TEST_F(BatchMatrixMultiplyTest,
  ...
  const auto height = 4;           // shape_rng();
  const auto input_channels = 15;  // channels_rng();
  const auto output_channels = 6;  // channels_rng();
  auto xnnpack_delegate = get_delegate();
...
TEST_F(BatchMatrixMultiplyTest,
  ...
  const auto height = 4;            // shape_rng();
  const auto input_channels = 5;    // channels_rng();
  const auto output_channels = 16;  // channels_rng();
  auto xnnpack_delegate = get_delegate();
