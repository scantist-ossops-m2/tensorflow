...
TEST(ODMLSDPA, MQA) {
  ...
  ODMLSDPATester()
      .Input1Shape({batch, input_seq_len, q_heads, head_dim})  // q
      .Input2Shape({batch, max_seq_len, kv_heads, head_dim})   // k
      .Input3Shape({batch, max_seq_len, kv_heads, head_dim})   // v
      .Input4Shape({batch, 1, input_seq_len, max_seq_len})     // mask
      .Input5Shape({1})                                        // scale
