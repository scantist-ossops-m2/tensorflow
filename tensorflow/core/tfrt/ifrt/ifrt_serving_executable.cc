...
absl::StatusOr<xla::HostCallback> BuildHostCallback(
  ...
  for (const auto& metadata : builder_info.device_to_host.metadata()) {
    TF_ASSIGN_OR_RETURN(
        xla::Shape shape,
        TensorShapeToXlaShape(metadata.type(), metadata.shape()));
  ...
  host_callback.callback = [function_defs, operand_type_and_shapes,
                            result_type_and_shapes,
                            entry_function_name = std::string(key),
                            device_mgr](void** output, void** input) {
    TF_ASSIGN_OR_RETURN(
        auto host_callback,
        TfHostCallback::Create(function_defs, entry_function_name,
                               operand_type_and_shapes, result_type_and_shapes,
                               device_mgr));
    return host_callback->Call(input, output);
  };
...
absl::StatusOr<std::vector<xla::HostCallback>> BuildHostCallbacks(
  ...
  for (const auto& [entry_function, builder_info] : host_callback_maps) {
    TF_ASSIGN_OR_RETURN(
        auto host_callback,
        BuildHostCallback(entry_function, builder_info, module, device_mgr));
...
IfrtServingExecutable::CreateExecutableSynchronously(
  ...
  absl::flat_hash_map<std::string, std::shared_ptr<tensorflow::EagerContext>>
      tf_executable_ctxs;

  TF_ASSIGN_OR_RETURN(std::shared_ptr<tensorflow::StaticDeviceMgr> device_mgr,
                      CreateTfStaticDeviceMgr());

  TF_ASSIGN_OR_RETURN(auto host_callbacks,
                      BuildHostCallbacks(tf2hlo_result, *module_, device_mgr));
  ...
  // xla::PjRtHostSendAndRecvLoadedHostCallback
  ...
  executable_bundle.device_mgr = std::move(device_mgr);
