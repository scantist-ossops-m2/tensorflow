...
HloControlFlowFlattening::RemoveRecvDone(
...
absl::StatusOr<std::pair<HloInstruction*, HloInstruction*>>
HloControlFlowFlattening::RemoveSendDone(
    ...
    for (HloInstruction* instruction :
      ...
      if (flatten_while_loop_ && instruction->opcode() == HloOpcode::kWhile) {
        ...
        if (remove_comm_ || (remove_host_transfer_ &&
          ...
          VLOG(1) << "Remove " << instruction->name();
          TF_RETURN_IF_ERROR(RemoveSendDone(instruction, &removed).status());
          changed = true;
        }
        ...
        if (remove_comm_ || (remove_host_transfer_ &&
          ...
          VLOG(1) << "Remove " << instruction->name();
          TF_RETURN_IF_ERROR(RemoveRecvDone(instruction, &removed).status());
          changed = true;
        }
