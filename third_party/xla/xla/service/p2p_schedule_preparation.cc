...
// For an pipelined Send-Recv chain involving two channels, adds control
// dependence to enforce this ordering:
...
absl::StatusOr<bool> P2PSchedulePreparation::Run(
  ...
  for (auto iter = all_computations.rbegin(); iter != all_computations.rend();
    ...
    if (result.second != nullptr) {
      // The current computaion is a while-body with pipelined P2P chain.
