...
TEST_F(WhileDoubleBufferRemovalTest, RemoveDoubleBuffer) {
  ...
  Literal reference = ExecuteAndTransfer(module->Clone(), {});
  std::cout << "before:\n" << reference.ToString() << std::endl;
  ...
  Literal actual = ExecuteAndTransfer(module->Clone(), {});

  ASSERT_TRUE(LiteralTestUtil::NearOrEqual(/*expected=*/reference,
                                           /*actual=*/actual, std::nullopt));
}

TEST_F(WhileDoubleBufferRemovalTest, RemoveDoubleBuffer2) {
  [[maybe_unused]] constexpr char kModule[] = R"(
  HloModule jit_scan
...
TEST_F(WhileDoubleBufferRemovalTest, BufferAllocateOutside) {
  [[maybe_unused]] constexpr char kModule[] = R"(
  HloModule jit_scan
...
TEST_F(WhileDoubleBufferRemovalTest, InputDifferentShape) {
  [[maybe_unused]] constexpr char kModule[] = R"(
  HloModule jit_scan
...
TEST_F(WhileDoubleBufferRemovalTest, MultipleUsersInput) {
  [[maybe_unused]] constexpr char kModule[] = R"(
  HloModule jit_scan
...
TEST_F(WhileDoubleBufferRemovalTest, RemoveDoubleBufferMoreInputs) {
  [[maybe_unused]] constexpr char kModule[] = R"(
  HloModule jit_scan
  ...
  // Index 2 of the while is replaced with the input array.
  for (HloInstruction* instr :
       module->entry_computation()->MakeInstructionPostOrder()) {
    if (instr->opcode() == HloOpcode::kWhile) {
      EXPECT_EQ(instr->while_init()->operand(2)->opcode(),
